...
封装 Spark 4.0 Ultra / X2 的 WebSocket API，支持流式与非流式调用。
当环境变量 MOCK_LLM=true 时切换到 mock 模式，根据输入动态生成合理响应。
"""
import asyncio
import base64
import hashlib
...
# WebSocket 库做软依赖：未安装时降级运行（仅 mock 模式可用）
try:
...
class SparkClient:
    ...
    async def _call_spark(
        ...
        async with websockets.connect(auth_url, ping_interval=None, close_timeout=10) as ws:  # type: ignore
            ...
            async for raw in ws:
                ...
                if code != 0:
                    raise SparkAPIError(f"Spark 错误 code={code}: {data.get('header', {}).get('message')}")
    ...
    async def _mock_chat(
        ...
        策略: 解析 messages 中最后一条 user 消息，识别任务类型 (jd_parse/role_definition/resume_parse/skill_extract)
    ...
    def _infer_task(user_msg: str) -> str:
        text = user_msg.lower()
        if "聚类信息" in user_msg or "簇 id" in user_msg.lower():
            return "role_definition"
        if "jd 原文" in user_msg or "招聘 jd" in user_msg or "schema 严格输出" in user_msg:
            return "jd_parse"
        if "简历文本" in user_msg or "候选人简历" in user_msg:
            return "resume_parse"
        if "技术技能名词" in user_msg or "识别" in user_msg and "技能" in user_msg:
            return "skill_extract"
        return "generic"

    @staticmethod
    def _extract_jd_text(user_msg: str) -> str:
        match = re.search(r"【JD 原文】\s*(.+?)\s*【输出】", user_msg, re.DOTALL)
        if match:
    ...
    def _mock_role_response(self, user_msg: str) -> str:
        ...
        confidence = round(
            sum(s["weight"] for s in required) / max(len(required), 1) * 0.9, 2
        )
        result = {
    ...
    def _detect_level(jd_text: str) -> str:
        if "资深" in jd_text or "专家" in jd_text or "Staff" in jd_text or "Principal" in jd_text:
            return "资深"
        if "高级" in jd_text or "Senior" in jd_text or "Sr." in jd_text:
            return "高级"
        if "中级" in jd_text or "Intermediate" in jd_text or "P5" in jd_text or "P6" in jd_text:
            return "中级"
        if "初级" in jd_text or "Junior" in jd_text or "应届" in jd_text or "实习" in jd_text:
            return "初级"
        return "中级"
    ...
    def _extract_responsibilities(jd_text: str) -> List[str]:
        ...
        for line in jd_text.splitlines():
            ...
            if not s:
                continue
            if any(s.startswith(k) for k in keywords) or (len(s) > 10 and len(s) < 120 and "。" in s):
                duties.append(s[:200])
            if len(duties) >= 6:
                break
        if not duties:
    ...
    def _extract_skills(jd_text: str, kind: str) -> List[Dict[str, Any]]:
        ...
        required_keywords = ["必须", "要求", "必备", "Required", "Must have", "Requirement"]
        preferred_keywords = ["优先", "加分", "Preferred", "Nice to have", "Plus"]
        ctx_kind = "required"
        for line in jd_text.splitlines():
            if any(k in line for k in preferred_keywords):
                ctx_kind = "preferred"
            if any(k in line for k in required_keywords):
                ctx_kind = "required"
            for token in tech_keywords:
                if token in line and (ctx_kind == kind or (kind == "required" and ctx_kind == "required")):
                    pass
        found = []
        for token in tech_keywords:
            if token in jd_text:
                # 根据上下文决定 required/preferred
